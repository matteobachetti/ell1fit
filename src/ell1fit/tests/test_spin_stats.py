...
from ..spin_periodicity import floating_mean_periodogram, rate_values, search_periodicity
...
def test_cli_secular_and_local_spin(tmp_path):
    ...
    assert set(summary["periodicity"]) == {"f1", "pulsed_rate"}
    assert 50 <= summary["periodicity"]["f1"]["best_period_days"] <= 70
    for suffix in ("_epochs.ecsv", "_trend.png", "_f1.png", "_periodogram.png", "_folded.png"):
